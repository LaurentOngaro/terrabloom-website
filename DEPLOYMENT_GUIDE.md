...
Ce guide explique comment déployer le site TerraBloom sur GitHub Pages avec les domaines personnalisés achetés chez OVH.

## 0 - informations préalables

- Terrabloom Google Analytics Measurement ID : G-8LMY4GLCC9
...
3. Cliquer sur **Save**
4. (Optionnel mais recommandé) Renseigner le domaine personnalisé `playterrabloom.com` puis cocher **Enforce HTTPS**. GitHub ajoutera/actualisera automatiquement le fichier `CNAME` dans `/docs/`.
5. DevLog : le site lit les articles via `docs/data/devlog.json`. Générer/mettre à jour ce fichier avec `python _Helpers/03_Maintenance/generateWebsite.py --publish` (ou sans `--publish` en mode dev), puis commit/push le JSON.

Le site sera accessible soit via le domaine personnalisé `https://playterrabloom.com` (après propagation DNS), soit via l'URL par défaut GitHub Pages `https://username.github.io/terrabloom-website`.
...
- [x] Naviguer vers http://localhost:8000 (ou votre port)
- [x] Vérifier la page d'accueil (index.html) s'affiche correctement
- [x] Tester les liens de navigation (Game, About, Contact, Privacy)
- [x] Vérifier que tous les styles CSS s'appliquent
- [x] Vérifier la console navigateur pour les erreurs (F12 → Console)
...
```html
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-8LMY4GLCC9"></script>
<script>
...
- [x] Dépôt GitHub créé et configuré
- [x] Fichiers HTML dans `/docs/`
- [x] CSS dans `/docs/css/`
- [x] JavaScript dans `/docs/js/`
- [ ] Favicon placé dans `/docs/'
- [x] GitHub Pages activé avec source `/docs`
- [x] Domaine personnalisé configuré dans GitHub
- [x] Enregistrements DNS configurés dans OVH
- [x] DNS propagé (vérifier avec `nslookup`)
- [x] Formspree IDs remplacés dans tous les formulaires
- [x] HTTPS forcé activé dans GitHub Pages
- [x] Site accessible via `playterrabloom.com`
- [x] Site accessible `www.playterrabloom.com`
- [ ] Site accessible via `terrabloom.dev`
- [ ] Site accessible `www.terrabloom.dev`
